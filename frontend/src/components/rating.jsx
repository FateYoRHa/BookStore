...
const Rating = ({
    ...
    if (hasHalfStar) {
      stars.push(
        <div key="rating-half-star" className="relative">
          <StarHalf className="absolute top-0 right-0 fill-accent stroke-foreground" />
  ...
  if (showScore && !description) {
    return (
      <div
        className={cn('flex items-center gap-2 [&_svg]:size-5 [&>div]:size-5', className)}>
        {renderStars()}
  ...
  return (
    <div
      className={cn('flex items-center gap-1 [&_svg]:size-5 [&>div]:size-5', className)}>
      {renderStars()}
    </div>
  );
};
